...
\title{%
    ...
    \normalsize Zimbabwe Occupational Pension Funds (2018--2022)
}
...
workers across public and private employment.  The sector is regulated by the Insurance
and Pensions Commission (IPEC) and operates under extreme macroeconomic stress:
hyperinflation peaked above 500\% in 2020, multiple currency reforms were enacted
between 2018 and 2022, and COVID-19 disrupted member contributions and investment
markets simultaneously.

\textbf{The central question of this study is:} \emph{Among Zimbabwean occupational
pension funds, which funds convert their resources into member benefits most
efficiently, and what structural or environmental factors explain differences in
efficiency?}
...
\begin{enumerate}[leftmargin=2cm]
    ...
    \item \textbf{Stage 2 — Determinant analysis (Random Forest regression).}
          Once bias-corrected efficiency scores are obtained, we regress them on
          environmental variables (inflation, exchange rate volatility, fund age) to
          identify which contextual factors drive efficiency differences.
\end{enumerate}

The pipeline also performs:
\begin{itemize}
    \item \textbf{PCA} on the input variables to check multicollinearity and
          optionally reduce dimensionality before DEA.
...
The analysis uses a panel dataset of 20 Zimbabwean pension funds observed annually
from 2018 to 2022 (100 rows total).  The file is \texttt{tests/dataset.csv}.
...
\bottomrule
\endfoot
fund\_id          & string  & Unique fund identifier (\texttt{ZW001}--\texttt{ZW020}).
...
fund\_name        & string  & Full name of the fund (e.g.\
                              \textit{NationalSocialSecurityAuthority}).
                              Constant within a fund. \\[4pt]
fund\_type        & string  & \texttt{public} (government-sponsored) or
                              \texttt{private} (employer-sponsored occupational scheme). \\[4pt]
...
inflation         & float & Annual CPI inflation rate for Zimbabwe (\%).
                            ...
                            \textbf{Environmental variable (Stage 2).} \\[4pt]
exchange\_volatility & float & Annualised standard deviation of the ZWL/USD exchange
...
fund\_age         & integer & Years since the fund's inception as of the observation
                              ...
                              \textbf{Environmental variable (Stage 2).} \\
\end{longtable}
...
\begin{itemize}
    ...
    \item \textbf{Environmental variables} are not under the fund manager's control
          (inflation, exchange rate) or only slowly controllable (fund age, size tier).
          They are excluded from the DEA frontier but analysed in Stage 2.
\end{itemize}
...
% =============================================================================
\section{Stage 1A: PCA Input Validation}
% =============================================================================

\subsection{Purpose}

Before running DEA we apply Principal Component Analysis (PCA) to the three input
variables.  The goals are:
\begin{enumerate}
    \item Check for multicollinearity (if $\geq 95\%$ of variance is explained by a
          single component, the inputs are near-collinear and should be consolidated).
    \item Optionally replace the three raw inputs with two PCA composites to satisfy
          the DEA rule of thumb $n \geq 3(m + s)$, where $n$ is the number of DMUs,
          $m$ the number of inputs, and $s$ the number of outputs.
\end{enumerate}

\subsection{Calculation}

Let $\mathbf{X} \in \mathbb{R}^{n \times m}$ be the input matrix ($n = 20$ funds,
$m = 3$ inputs).

\textbf{Step 1 — Standardisation.}  Compute the standardised matrix:
\[
    \tilde{x}_{ij} = \frac{x_{ij} - \bar{x}_j}{s_j}
\]
where $\bar{x}_j$ and $s_j$ are the column mean and standard deviation.
This ensures financial columns measured in different magnitudes contribute equally.

\textbf{Step 2 — Covariance / correlation.}  The correlation matrix is:
\[
...
\]
where $\mathbf{\Lambda} = \operatorname{diag}(\lambda_1, \ldots, \lambda_m)$ are
eigenvalues in descending order and $\mathbf{V}$ contains the corresponding
eigenvectors (principal component loadings).

\textbf{Step 4 — Explained variance ratio.}
\[
    \text{EVR}_k = \frac{\lambda_k}{\sum_{j=1}^{m} \lambda_j}
\]

\textbf{Step 5 — Transform.}  The principal component scores:
\[
    \mathbf{Z} = \tilde{\mathbf{X}} \mathbf{V}_{:, 1:K}
    \quad (K \text{ components retained})
\]

\subsection{Scree plot interpretation}

The scree plot bar chart shows $\text{EVR}_k \times 100$ for each component.
A steep drop after the first component (``elbow'') indicates strong collinearity.
In our data the first component typically explains $\geq 80\%$ of variance, confirming
that the three input variables are highly correlated (all scale with fund size).

% =============================================================================
\section{Stage 1B: CCR (CRS) DEA Model}
% =============================================================================
...
% =============================================================================
\section{Stage 1C: BCC (VRS) DEA Model}
% =============================================================================
...
% =============================================================================
\section{Stage 1D: Scale Efficiency and Returns to Scale}
% =============================================================================
...
\begin{itemize}
    \item Large public funds (NSSA, civil service) $\rightarrow$ DRS
    \item Small agriculture/university funds $\rightarrow$ IRS
    \item Medium efficient funds $\rightarrow$ CRS
\end{itemize}
...
% =============================================================================
\section{Stage 1E: Peer Analysis and Input Reduction Targets}
% =============================================================================
...
% =============================================================================
\section{Stage 2A: Simar--Wilson Bootstrap Bias Correction}
% =============================================================================
...
% =============================================================================
\section{Stage 2B: Random Forest Second Stage}
% =============================================================================

\subsection{Purpose}

The second-stage regression identifies which environmental factors
(inflation, exchange volatility, fund age) explain variation in
bias-corrected efficiency scores $\tilde{\theta}_k$.

\subsection{Model}

\[
    \tilde{\theta}_k = f(\text{inflation}_k,\; \text{exchange\_volatility}_k,\; \text{fund\_age}_k) + \varepsilon_k
\]

$f(\cdot)$ is estimated by a Random Forest with:
\begin{itemize}
    \item $n\_\text{estimators} = 500$ trees
    \item $\text{max\_features} = \sqrt{p}$ features per split
...
\subsection{PCA Scree Plot}

A bar chart with principal components on the x-axis and explained variance (\%)
on the y-axis.  \textbf{What to look for:} if PC1 explains $>80\%$, the three
inputs are nearly collinear (all scale with fund size).  In that case, using
PCA composites (\texttt{--use-pca-composite}) is recommended to avoid the
``curse of dimensionality'' in the DEA LP.
...
def test_dataset_schema():
    df = pd.read_csv("tests/dataset.csv")
    ...
    # Macro variables match expected values per year
    expected = {2018: 10.6, 2019: 255.3, 2020: 557.2, 2021: 98.5, 2022: 243.8}
    for year, inf in expected.items():
        vals = df.loc[df["year"] == year, "inflation"].unique()
        assert len(vals) == 1 and abs(vals[0] - inf) < 0.2, \
...
\subsection{Stage 1: PCA variance explained}

\begin{lstlisting}[style=python, caption={Test: PCA retains >= 95\% variance in 3 components}]
import numpy as np
import pandas as pd
...
def test_pca_variance():
    ...
    # Three components should explain >= 95% of variance
    assert result.cumulative_variance[-1] >= 0.95, \
        f"PCA explains only {result.cumulative_variance[-1]:.2%} of variance"

    # First component dominates (collinearity test: EVR1 > 0.60)
    assert result.explained_variance_ratio[0] >= 0.60, \
        f"PC1 explains only {result.explained_variance_ratio[0]:.2%}"

    # Components must be finite
    assert np.isfinite(result.components).all()

    print(f"PASS: PC1={result.explained_variance_ratio[0]:.1%}, "
          f"Cumulative={result.cumulative_variance[-1]:.1%}")

test_pca_variance()
\end{lstlisting}

\subsection{Stage 2: CCR DEA correctness}
...
\subsection{Stage 3: BCC $\geq$ CCR and Scale Efficiency in (0, 1]}
...
def test_scale_efficiency():
    ...
    # All three classes should appear in a realistic dataset of 20 funds
    present = set(scale.rts_class)
    assert len(present) >= 2, f"Only {present} RTS classes observed"
...
\subsection{Stage 4: Input reduction targets are valid}
...
def test_targets():
    ...
    dataset = pd.read_csv("tests/dataset.csv")
    ...
    for col in INPUT_COLS:
        ...
        # Targets must be <= actual (efficiency <= 1 means contraction only)
...
\subsection{Stage 5: Bootstrap bias correction properties}

\begin{lstlisting}[style=python, caption={Test: bias-corrected scores are <= raw scores and CIs are valid}]
import numpy as np
import pandas as pd
from pension_toolkit.data_io import load_csv, get_dea_matrices
from pension_toolkit.dea_core import dea_ccr_input_oriented
...
def test_bootstrap():
    ...
    # CIs bracket the bias-corrected score (in most cases)
    inside = ((result.theta_bias_corrected >= result.ci_lower - 1e-4) &
              (result.theta_bias_corrected <= result.ci_upper + 1e-4))
    assert inside.mean() >= 0.80, \
        f"Only {inside.mean():.0%} of BC scores inside their CIs"
...
\subsection{Stage 6: Random Forest second stage}

\begin{lstlisting}[style=python, caption={Test: RF fits without error and importance sums to 1}]
import numpy as np
import pandas as pd
...
def test_rf_second_stage():
    ...
    # Load bias-corrected scores from a prior CLI run
    try:
        bc = pd.read_csv("out/bias_corrected_scores.csv")
    except FileNotFoundError:
        print("SKIP: run CLI pipeline first to generate out/bias_corrected_scores.csv")
        return

    bc = bc.merge(df_agg[["fund_id"] + ENV_FEATURE_COLS], on="fund_id")
    env_features = bc[ENV_FEATURE_COLS].to_numpy(dtype=float)
    scores = bc["theta_bias_corrected"].to_numpy(dtype=float)

    result = fit_rf(scores, env_features,
                    feature_names=ENV_FEATURE_COLS, seed=42)
    ...
    # Permutation importance has correct shape
    assert len(result.permutation_importance) == len(ENV_FEATURE_COLS)
...
\subsection{Stage 7: End-to-end pipeline sanity check}
...
# 2. Run full analysis (use real dataset, 200 bootstrap replications for speed)
uv run python -m pension_toolkit.cli analyze \
    --input tests/dataset.csv \
    --out out/ \
    --bootstrap-B 200 \
    --seed 42
